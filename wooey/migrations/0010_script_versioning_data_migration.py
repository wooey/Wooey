...
def make_script_versions(apps, schema_editor):
    ...
    scripts = sorted(
        [(i.script_name, i) for i in Script.objects.all()], key=itemgetter(0)
    )
    for script_name, scripts in groupby(scripts, key=lambda x: x[0]):
        ...
        ordered_scripts = sorted(
            [(int(script.script_version), script) for i, script in scripts],
            key=itemgetter(0),
        )
        ...
        for i, v in enumerate(ordered_scripts):
            ...
            version_kwargs = {
                "script_version": "1",
                "script_iteration": script.script_version,
            ...
            script_version.save()
            ScriptParameter.objects.filter(script=script).update(
                script_version=script_version
            )
            ScriptParameterGroup.objects.filter(script=script).update(
                script_version=script_version
            )
