...
class WooeyJob(models.Model):
    ...
    def get_resubmit_url(self):
        return reverse(
            ...
            kwargs={"slug": self.script_version.script.slug, "job_id": self.pk},
        )
...
class ScriptParameter(models.Model):
    ...
    custom_widget = models.ForeignKey(
        "WooeyWidget", null=True, blank=True, on_delete=models.SET_NULL
    )
...
class ScriptParameters(models.Model):
    ...
    def value(self, value):
        ...
        if field == self.CHAR:
            ...
            value = (
                self.WOOEY_FIELD_MAP[field](value)
        ...
        if add_file:
            ...
            wooey_file, file_created = WooeyFile.objects.get_or_create(
                checksum=checksum
            )
            if file_created:
